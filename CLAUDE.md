...
- **`cargar_contexto`** (code) loads the `Agent` (+`user`), the `Conversation`, the active `Turno`, and calls `freeBusy` **once** over the next `DIAS_VENTANA` (14) days, merging into it the owner's own confirmed `Turno` rows (`unirOcupados`) — a free DB read that covers what `freeBusy` hasn't propagated yet and events the owner marked "Disponible", which `freeBusy` never reports as busy. It turns that into a compact per-day list of free gaps that goes straight into the system prompt, alongside the generated `systemPrompt`, the hard-rules block, the scope block and the style block (`reglasDeAgenda` / `reglasDeAlcance` / `reglasDeEstilo`), the first two built from the `Agent` row. Only `DIAS_RESUMEN` (5) working days of that window reach the prompt — weekends are skipped entirely, not printed as "sin huecos". That snapshot is why a whole conversation costs one Google read instead of one per tool call.
...
The four agenda rules — weekdays only, no overlapping, at least `MARGEN_MINIMO_MIN` (5) minutes between turnos, and always asking the client's name — live in `graph/agenda-rules.ts` as pure functions and are enforced by the `validacion` node, not only by the generated prompt: `crear_turno` requires a `nombreCliente`, both it and `reprogramar_turno` reject anything outside the `Agent`'s `horaDesde`/`horaHasta` or falling on a Saturday or Sunday (`esDiaHabil`, the `DIAS_HABILES` set — deliberately a constant and not an `Agent` column, so it applies to already-generated agents with no migration), and collisions are checked over the range widened by the margin (`conMargen`). `huecosDelDia` returns nothing for a non-working day, which is the single choke point that keeps weekends out of the prompt summary, out of `horariosCercanos` suggestions and out of the day pool. A collision rejection carries the fix, not just the refusal: `mensajeOcupado` appends the nearest free slot found by `horariosCercanos`, which walks the loaded window through `huecosDelDia` and picks, inside each gap, the point closest to what was asked (asking for 10:03 right after a turno ending at 10:00 suggests 10:05, not the 09:00 start of the gap). `parsearFecha` anchors any ISO string without an offset to `TIMEZONE` before `new Date`: the API container runs in UTC, so "14:00" used to become 11:00 ART and every collision check ran against the wrong instant. `reprogramar_turno` filters the turno's own event out of the busy periods (`conflictos`), otherwise the margin makes a small move collide with itself. The rules block in the system prompt is built from the `Agent` row (not from the generated prompt, so it survives regeneration), and `Conversation.nombreCliente` is reused so the agent doesn't ask for the name twice. `reglasDeAlcance` (same file, same reason it lives in code) is what keeps the agent on topic: it may only talk about turnos and the business data already in the `Agent` row (attention window, tipos de turno, who attends), it must refuse anything else in one line — including an off-topic question smuggled into a booking message — and it must never invent business data it wasn't given (prices, address), deferring to the owner instead. The meta-agent's `META_SYSTEM_PROMPT` asks for the same restriction so a freshly generated prompt agrees with it, but the code block is the guarantee and covers agents generated before it existed, with no regeneration. `reglasDeEstilo` (in `cargar-contexto.node.ts`, same reasoning) is the third code-owned block: WhatsApp-length answers, no markdown, never more than `MAX_OPCIONES_DIA` (3) times per message. The validation node pushes the same way — a single-day `consultar_disponibilidad` (`esConsultaDeUnDia`) is answered with `resumenDelDia` instead of the busy-period dump: a free day comes back as a range ("libre de 09:00 a 18:00"), a day that already has turnos comes back as up to three spread-out times from `opcionesDelDia` plus the instruction to ask "¿mañana o tarde?" first. A `CalendarUnavailableError` (revoked/missing token, Google API failure) turns into an apology instead of crashing the graph; if it happens while loading the snapshot, the validation node refuses to book anything. `LIMITE_RECURSION` (14 supersteps) replaces the old 4-round-trip cap.

**The free month is a rule, not a row.** Every user gets `DIAS_PRUEBA` (30) days of everything, counted from `User.createdAt` — the Google login — with no card asked for. `api/src/subscription/subscription.rules.ts` holds that as pure functions (`finDePrueba`, `estadoDeSuscripcion`), the same way `agenda-rules.ts` holds the agenda rules: deriving the trial instead of storing it means users created before billing existed need no backfill, and `estado` (`activa | prueba | por_vencer | vencida`, the last two split at `DIAS_AVISO` = 7) is computed the same way everywhere. A `Subscription` row only appears once a payment starts.

When the month runs out and nobody paid, **only the WhatsApp assistant goes quiet** — the agenda, the calendar and every screen stay open, which is what the copy promises. The cut is `estadoDeSuscripcion(...).asistenteActivo`, checked at the two doors into the graph: `WhatsappService.handleMessagesUpsert` skips the message without answering (the Baileys session stays linked, so paying brings the bot back with no new QR), and `ConversationController.test` — the Home test bench, where the reader is the owner — answers `MENSAJE_SUSCRIPCION_VENCIDA` instead of silence.

Payment is Mercado Pago's `preapproval` API and nothing else: `POST /suscripcion/checkout` creates it with `status: 'pending'` and hands back the `init_point` the browser is redirected to, so no card data ever reaches this codebase. `mercadopago.client.ts` is a `fetch` wrapper shaped exactly like `openrouter.client.ts` (no SDK, no axios). `POST /suscripcion/webhook` is the only unguarded `POST` in the API: it verifies the `x-signature` HMAC (`webhook-signature.ts`, built from headers + the `data.id` query — **not** the raw body, which is why `main.ts` needs no `rawBody`), then ignores the payload's contents and re-reads the preapproval from Mercado Pago before upserting by `mpPreapprovalId`, so repeated deliveries are idempotent. It always answers `200`; a 5xx would put MP into a retry loop.

On the web side `SuscripcionProvider` (`web/src/lib/suscripcion.tsx`) fetches `GET /suscripcion` once per session and is what `AvisoSuscripcion` (banner on `/inicio`, `/calendario`, `/chat`; bottom sheet on mobile when vencida) and `/plan` read. `/plan` redirects to Mercado Pago and comes back at `?volviendo=1`, where it polls `GET /suscripcion` a few times because the webhook can land after the browser does. Note the lint rule the app enforces: `setState` may not be called in an effect body, so both the provider and that polling set state inside promise callbacks.
...
Cookies are not scoped by port, so `:4000` and `:3000` share `localhost` and `sameSite: 'lax'` is enough locally. Production is on separate Railway domains, so `cookieOptions` already switches to `sameSite: 'none'` + `secure` when `NODE_ENV === 'production'` — with the known cost that Safari/iOS blocks third-party cookies outright, so the login only works there once front and API share a domain (own subdomains, or a Next `/api/*` rewrite). Losing `lax` also loses the browser's CSRF guard, so `main.ts` mounts `chequeoDeOrigen` (`api/src/auth/csrf-origin.ts`): any state-changing method whose `Origin` isn't `FRONTEND_URL` gets a 403 before reaching a controller. A missing `Origin` is allowed on purpose — that is how Mercado Pago's webhook arrives, and it authenticates by HMAC, not by the cookie.
