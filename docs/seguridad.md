...
| 5 | Media | Las imágenes de producción corrían como `root`. | **Arreglado**: `USER node`; el código queda de root (sólo lectura) y en la web sólo `.next` es escribible (cache de `next/image`). CI verifica que la imagen no sea root. |
...
2. *Settings → Code security*: prender Dependabot alerts, secret scanning y
